...
def get_measurement_list():
    """"
    This function returns a list of light sensor reading in a physically dimensionally accurate list
    """
...
def weight_measurement_list(measurement_list):
    """
    This function gives weights to a list of measurements equal to the sensor distances from the centre with +ve being right
    """
...
def calc_error(weighted_measurement_list, setpoint=0):
    """
    This function calculates the distance from the centre then calculates the error from a setpoint which is zero in this case
    """
...
def motor_control(control_signal, speed = 50):
    """
    This function controls motors proportionally to the control signal around a base speed
    """

    k = 1 #amplifier constant
    if control_signal < 0:
...
def junc_detection(measurement_list):
    """
    This function filters out erroneous junction detections by providing a minimum number of times that a junction must be detected
    """
...
def line_following(pickup = False, blind=False, blind_time=0):
    """
    This function follows a line either for a set time or until a junction is detected
    """
    ...
    while state == "not at junction":
        ...
        if error == None:
            error = prev_error
        results = calc_control_signal(error, prev_error, prev_integrator, prev_differentiator, T = 0.05)
...
def blind_reverse(distance_wanted):
    """
    This function makes the robot move forwards a certain distance without following the line
    """
